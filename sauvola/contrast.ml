...
let _ =
        try
                ...
                save_image_as (outfile_prefix^"__adaptive_contrast_spreading.png") (img |> adaptive_contrast_spreading);
                let start1 = Unix.gettimeofday () in
                Printf.printf "%f s adaptive contrast spreading\n" (start1 -. start0);
                save_image_as (outfile_prefix^"__niblack_global_gray.png") (img |> niblack_global_contrast_maximization);
                let start2 = Unix.gettimeofday () in
                Printf.printf "%f s niblack global gray\n" (start2 -. start1);
                 save_image_as (outfile_prefix^"__niblack_local_gray.png") (img |> niblack_local_contrast_maximization);
                let start3 = Unix.gettimeofday () in
                Printf.printf "%f s niblack local gray\n" (start3 -. start2);
                save_image_as (outfile_prefix^"__niblack_local_monochrome.png") (img |> niblack_local_monochromize);
                let start4 = Unix.gettimeofday () in
                Printf.printf "%f s niblack local mono\n" (start4 -. start3);
                save_image_as (outfile_prefix^"__sauvola_global_gray.png") (img |> sauvola_global_contrast_maximization); 
                let start5 = Unix.gettimeofday () in
                Printf.printf "%f s sauvola global gray\n" (start5 -. start4);
                save_image_as (outfile_prefix^"__sauvola_global_monochrome.png") (img |> sauvola_global_monochromize);
                let start6 = Unix.gettimeofday () in
                Printf.printf "%f s sauvola global mono\n" (start6 -. start5);
                save_image_as (outfile_prefix^"__sauvola_local_gray.png") (img |> sauvola_local_contrast_maximization);
                let start7 = Unix.gettimeofday () in
                Printf.printf "%f s sauvola local gray\n" (start7 -. start6);
                save_image_as (outfile_prefix^"__sauvola_local_monochrome.png") (img |> sauvola_local_monochromize);
